...
class rpsAI {
	...
	private int[] timesPlayed; // contains the number of times the player has played each of the three moves.
	...
	rpsAI() {// constructor for the AI class
		markovChain = new float[][] { { 0.33f, 0.33f, 0.33f }, { 0.33f, 0.33f, 0.33f }, { 0.33f, 0.33f, 0.33f } };// Didn't
																													// really
																													// see
																													// a
																													// need
	...
	public String makeMove() {// main function for the class, probabilistically guesses the player's next
								// move, then counters it
		Random rand = new Random();// establish a random number generator
		float ranFloat = rand.nextFloat();// generate a random float
		if (ranFloat <= markovChain[lastMove][1]) {// use that random float, along with our Markov Chain values of
													// potential moves, to generate a probabilisically determined move
			return "paper";// Note that the returned moves are, in fact, the counters to the move the AI
							// predicts the user to perform
		} else if (ranFloat <= markovChain[lastMove][2] + markovChain[lastMove][1]) {// We add these two values together
	...
	public void update(String newMove) {// Takes in the results of the last game played, and uses them to update the
										// Markov chain using relevant data
		...
		/*
		 * 1. Multiply everything in the appropriate column of the Markov Chain by
		 * timesPlayed[moveBeforeLast]
		 * 2. Increment the row value we want (that is,
		 * markovChain[moveBeforeLast][lastMove] by one
		 * 3. Increment timesPlayed[moveBeforeLast] by one
		 * 4. Divide all values in markovChain[moveBeforeLast][x] by
		 * timesPlayed[moveBeforeLast]
		 */

		for (int i = 0; i < 3; i++) { // 1. Multiply everything in the appropriate column of the Markov Chain by
										// timesPlayed[moveBeforeLast]
		...
		// For debug purposes, let's go ahead and print the contents of this Markov
		// Chain:
		System.out.println("New Markov Chain");
		System.out.println("Rock to Rock: " + markovChain[0][0] + " Rock to Paper: " + markovChain[0][1]
				+ " Rock to Scissors: " + markovChain[0][2]);
		System.out.println("Paper to Rock: " + markovChain[1][0] + " Paper to Paper: " + markovChain[1][1]
				+ " Paper to Scissors: " + markovChain[1][2]);
		System.out.println("Scissors to Rock: " + markovChain[2][0] + " Scissors to Paper: " + markovChain[2][1]
				+ " Scissors to Scissors: " + markovChain[2][2]);

	}

	public void saveData(Scanner keyboard) {// Function which saves the Markov chain developed by the AI to a text file
											// for later use
		...
		String workingDirectory = currentRelativePath.toAbsolutePath().toString();// This directory is then converted
																					// into an absolute path, then into
																					// a string
		String newFilePath = workingDirectory + "/" + filename;// this string then has the new file appended to the end
																// (TODO: I think I fucked this up? Is this pointing to
																// a file or directory?)
		...
		try {
			...
			for (int k = 0; k < 3; k++) {// and we also print the contents of timesPlayed to it as well, so our future
											// loaded AI can properly update its inherited Markov Chain
				output.print(timesPlayed[k]);
				output.print(" ");
			}
			output.close();
		} catch (FileNotFoundException e) {// I don't think this will ever trigger, but in case it does, this exception
											// is caught
	...
	public void loadData(Scanner keyboard) {// A function which reads in a file and reconstructs the described Markov
											// Chain
		...
		try {
			...
			input.close();
		} catch (FileNotFoundException e) {// if we can't find the file the user's talking about, let 'em know, and exit
											// the function
			System.out.println("That file was not found.");
		}
	}

	private void setChain(int x, int y, float probability) {// simple setter routine, used to generate a Markov Chain
															// from a file
...
public class game {
	public static void main(String[] args) {
		...
		do {
			...
			int choice = Integer.parseInt(keyboard.nextLine());// read the user's input, then try to parse an int from
																// it, representing the three menu options
			...
			switch (choice) {// Now that we've verified input, this switch sends the user to the proper
								// function given their request.
				case 1:
	...
	static void play(Scanner keyboard, rpsAI opponent) {// play() forms the core gameplay loop of the game
		...
		do {// This do-while loop is, literally, the gameplay loop
			...
			String playermove = keyboard.nextLine().trim().toLowerCase();// prompt the user for a move, then correct it
																			// to proper style

			while (playermove.equals("rock") && playermove.equals("paper") && playermove.equals("scissors")) {// if the
																												// user's
																												// input
																												// isn't
																												// rock,
			...
			if (result == 0) {// then, given the result of results, print out a message describing the outcome
								// of the game
			...
			System.out.println("What would you like to do?");// Now, offer the user options as to what they want to do
																// next
			...
			int choice = Integer.parseInt(keyboard.nextLine());// Have them enter a numeric choice from the menu, then
			...
			switch (choice) {
				case 1:// Case 1 simply has them traverse this do-while loop again
					break;
				case 2:
					opponent.saveData(keyboard); // Case 2 tells the opponent to begin the process of saving it's
													// knowledge to a file
					...
					break;
				case 3:
					playing = false; // And Case 3 breaks this do-while loop, allowing the user to return to the main
										// menu
					break;
				default:
					break;
	...
	static int results(String playerMove, String AIMove) {// Results is an extremely simple function which returns the
															// results of a game of rock, paper, scissors
